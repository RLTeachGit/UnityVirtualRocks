...
public class SplittingBullet : BulletBase {
    ...
    protected override void UpdateMovement() {
        if(Timer<=0) {
            GameObject tGO1 = Instantiate(GM.singleton.HomingBulletPrefab,transform.position,Quaternion.identity);
            BulletBase tBullet1 = tGO1.GetComponent<BulletBase>();
            tBullet1.Velocity= Quaternion.Euler(0, 0, -Angle) * Velocity;

            GameObject tGO2 = Instantiate(GM.singleton.HomingBulletPrefab, transform.position, Quaternion.identity);
